...
describe("save-session MCP workflow", () => {
  ...
  it("shows repeated secret occurrences individually in the bulk decision form but still recaps unique counts", async () => {
    const token = `ghp_${"x".repeat(36)}`;
    const run = await setup("github-copilot-cli", accepted);
    await appendFile(run.fixture.primary, JSON.stringify({
    ...
    try {
      ...
      expect(result.isError).not.toBe(true);
      const decisionForm = ElicitRequestFormParamsSchema.parse(run.confirmations[0]!.params);
      expect(decisionForm.message).toContain("The scanner found 3 likely secrets");
      expect(decisionForm.message).toContain("1. [");
      expect(decisionForm.message).toContain("2. [");
      expect(decisionForm.message).toContain("3. [");
  ...
  it("asks once per finding via review-each and still recaps both raw and unique counts", async () => {
    let calls = 0;
    const run = await setup("github-copilot-cli", (request) => {
      calls++;
      if (calls === 1) return { action: "accept", content: { decision: "review-each" } };
    ...
    try {
      const result = await run.client.callTool({ name: "save_session", arguments: run.draft });
      expect(result.isError).not.toBe(true);
      // Secret-decision gate (review-each), two per-finding forms, the
      // metadata form, and the recap-confirm.
      expect(run.confirmations).toHaveLength(5);
      const perFinding = ElicitRequestFormParamsSchema.parse(run.confirmations[1]!.params);
      expect(perFinding.message).toContain("Finding 1 of 2");
      expect(perFinding.message).not.toContain("your decision applies to all of them");
      const secondFinding = ElicitRequestFormParamsSchema.parse(run.confirmations[2]!.params);
      expect(secondFinding.message).toContain("Finding 2 of 2");
      const recap = ElicitRequestFormParamsSchema.parse(run.confirmations[4]!.params);
  ...
  it("submits successfully with a blank additional-redactions answer, applying zero owner redactions", async () => {
    ...
    try {
      ...
      expect(result.isError).not.toBe(true);
      const form = ElicitRequestFormParamsSchema.parse(run.confirmations[0]!.params);
      expect(form.requestedSchema.required).not.toContain("additionalRedactions");
      const receipt = result.content[1];
      if (receipt?.type !== "text") throw new Error("Expected publication receipt");
