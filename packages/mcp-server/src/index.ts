...
import {
  readPublisherToken,
  writePublisherToken,
} from "./credentials/publisherCredentialStore.js";
import { acquireGithubToken } from "./credentials/githubTokenProvider.js";
...
const resolutionsSchema = z.array(z.object({
  ...
  action: z.discriminatedUnion("kind", [
    ...
    z.object({ kind: z.literal("owner-override-unredacted") }).strict(),
  ]),
}).strict()).max(10_000);
...
export function createDefaultBackendClient(
  ...
  /**
   * Resolved per request rather than captured once, so the token a first
   * publish mints is picked up by the very next call without restarting the
   * MCP server.
   *
   * `SESSION_REGISTRY_TOKEN` still wins when set, which keeps CI and other
   * non-interactive environments able to supply a credential explicitly
   * instead of depending on a file in somebody's home directory. It is no
   * longer required: its absence now means "not yet onboarded" rather than a
   * startup failure.
   */
  const getAccessToken = async (): Promise<string | null> => {
    const fromEnv = env.SESSION_REGISTRY_TOKEN?.trim();
    if (fromEnv !== undefined && fromEnv !== "") {
      return fromEnv;
    }
    return readPublisherToken(baseUrl, env);
  };
  ...
  return createHttpBackendClient({
    ...
    onPublisherTokenIssued: async (token) => {
      await writePublisherToken(baseUrl, token, env);
    },
    getGithubToken: async () => (await acquireGithubToken({ env })).token,
  });
}

function requiredEnvironmentValue(
  env: NodeJS.ProcessEnv,
  name: "SESSION_REGISTRY_API_URL",
): string {
  ...
  server.registerTool(
    "save_session",
    {
      ...
      description:
        ...
        "Always gathers session artifacts first, then always scans them for secrets and always reports how many were found. In interactive mode this is followed by a fixed, code-enforced sequence: (1) an explicit owner choice to redact all detected secrets, review and approve each one individually, or publish unredacted as an explicit override; (2) a one-shot metadata form (title, summary, audience, expiration, additional redactions) filled in exactly once, with no re-presentation on edits; (3) a separate, non-editable recap of every prior decision with a single yes/no confirmation before anything uploads. None of these steps can be skipped, merged, reordered, auto-approved, or inferred from the original publish request, regardless of how the agent or user phrases the request. " +
        "Set interactionMode to noninteractive ONLY when running headlessly with no possible further chat turn (Copilot -p, Claude --print, Codex exec): the user's publish prompt authorizes generated metadata, requested settings or defaults, owner-requested exact-text redactions, and the native-package warning without a form. Detected secrets not covered by an owner redaction still need explicit owner resolutions supplied via resolutions; the flow otherwise reports review-required and uploads nothing. " +
        "Set interactive when the user can respond, which includes every slash command and chat message in a live session \u2014 even a terse one like '/publish-session' or 'just publish it, this is a demo'. Never infer noninteractive from an explicit, urgent, or terse request; only the literal absence of a further chat turn (a flag-invoked headless process) justifies it. Never switch a cancelled interactive request to noninteractive. Do NOT construct an ask_user schema. " +
        "A fresh save always re-gathers session artifacts, even if you know a prior captureId; the server ignores an attempt to skip gathering by supplying a stale or foreign captureId and reports the freshly gathered one instead. If a step above returned a captureId because the connected client cannot render forms, reuse that same captureId on your next call; after an unknown publication result retry publish_session, never recapture.",
      inputSchema: z.object({
