...
function rejectZip(message: string): never {
  throw new NativeSessionArchiveError(`Cannot build ZIP archive: ${message}`);
}

/** A single named-byte-content source for {@link buildStoredZip}. */
export interface StoredZipSourceEntry {
  readonly path: string;
  readonly bytes: Buffer;
}

/**
 * Builds a deterministic, stored (uncompressed-only) ZIP from named byte
 * entries. This is the same non-ZIP64, safe-filename, NFC-collision-free
 * dialect that {@link buildNativeSessionBundle} produces and that
 * `nativeSessionBundleReader.ts`'s reader accepts, generalized so callers
 * other than native-bundle capture (e.g. re-packaging an already-verified
 * bundle to append an additional entry) can reuse it.
 */
export function buildStoredZip(sources: readonly StoredZipSourceEntry[]): Uint8Array {
  if (sources.length >= UINT16_MAX) rejectZip("too many files for a non-ZIP64 archive.");
  ...
  const entries = sources.map((source) => {
    ...
    if (parts.some((part) => /[<>:"|?*]/.test(part) || /[. ]$/.test(part) ||
      ...
      rejectZip(`unsafe extraction filename: ${path}`);
    }
    const key = path.normalize("NFC").toLowerCase();
    if (paths.has(key)) rejectZip(`duplicate extraction filename: ${path}`);
    paths.add(key);
    const name = Buffer.from(path, "utf8");
    if (name.toString("utf8") !== path) rejectZip("filename does not round-trip as UTF-8.");
    if (name.length > UINT16_MAX) rejectZip("filename exceeds the ZIP size limit.");
  ...
  for (const { key } of entries) {
    ...
    while (parts.length > 0) {
      if (paths.has(parts.join("/"))) rejectZip("a filename conflicts with a source directory.");
      parts.pop();
    }
  }
  ...
  if (size > UINT32_MAX || size > MAX_NATIVE_ARCHIVE_BYTES) rejectZip("ZIP exceeds the bundle size limit.");
