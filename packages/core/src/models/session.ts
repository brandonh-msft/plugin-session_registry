...
export interface Session {
  readonly id: string;
  /**
   * The publisher principal that created this snapshot. This is the
   * authoritative publisher identity: supersession scope, idempotency scope,
   * and MCP-side authorization all key off it, so an anonymous publisher
   * owns their own snapshots just as fully as a GitHub-bound one.
   */
  readonly publisherPrincipalId: string;
  /**
   * The GitHub login bound to the publisher at publish time, or null when
   * the session was published without a GitHub identity.
   *
   * This is provenance metadata only. It is never an authorization input:
   * ownership, supersession scope, and idempotency scope all key off
   * `publisherPrincipalId`, and audience access is decided by the share
   * link's own policy. Its sole consumer is the `manifest.json` entry added
   * to a native session download, which records who published the capture.
   */
...
/**
 * The audit-log actor for an action attributed to a session's publisher.
 *
 * Audit rows require a non-null actor, but a publisher may have no GitHub
 * identity at all. The publisher principal id is the durable identity in
 * that case, so it is used verbatim; it is already an opaque server-side
 * identifier and carries no personal information.
 */
export function sessionAuditActor(
  session: Pick<Session, "ownerGithubLogin" | "publisherPrincipalId">,
): string {
  return session.ownerGithubLogin ?? session.publisherPrincipalId;
}
