...
export const FULL_FIDELITY_SAVE_WORKFLOW = [
  ...
  "When the user can respond, set interactionMode:\"interactive\". save_session provides a primitive-only schema with prefilled editable title, summary, audience, expiration, and additional exact-text redactions plus unchecked confirmation/warning booleans. Audience is a string (anyone, users:alice,bob, org:github, team:org/team, repo:owner/name:read; semicolons join restricted rules), not a nested schema. Expiration accepts days, never or an ISO timestamp. Additional redactions are plain text, one rule per line: exact text alone, or \"exact text -> replacement\" for a custom replacement (default replacement is [REDACTED]); no JSON is required and matching is case-insensitive. If an interactive host lacks forms, show the returned filled-in proposal for plain-text confirm/edit \u2014 that plain-text exchange is still the mandatory questionnaire and must wait for the owner's actual reply before publish_session is ever called. Never turn decline, cancellation, timeout or missing elicitation support into headless authorization.",
...
export const FULL_FIDELITY_METADATA_DRAFTING_GUIDANCE = [
  ...
  "For interactive execution present one concise publish proposal with the drafted title and summary, audience, expiration, additional owner-requested redactions, capture, and native-package warning. Let the owner confirm or edit all proposed values rather than enter them from scratch. Warning acknowledgment and per-finding security decisions can be collected with the proposal. If the owner asks in plain language to redact additional text, translate that request into exact ownerRedactions and reconfirm the resulting proposal; scanner classification is not required. If its confirmation UI cannot prefill fields, show the proposal in text and offer confirm/edit instead of a blank form. Headless publication instead uses the explicit publish prompt as authorization for supplied or default settings, requested redactions, and the warning; unresolved detected secrets still block upload.",
...
export const FULL_FIDELITY_PUBLISH_PROMPT_CHECKLIST = [
  ...
  FULL_FIDELITY_METADATA_DRAFTING_GUIDANCE,
  "10. Obtain owner confirmation of the capture, each finding decision, any ownerRedactions:[{exactText,replacementText?,caseSensitive?}], title, summary, audience, and expiration, including the incomplete-detection disclaimer. Call publish_session with captureId, resolutions:[{findingId,action:{kind:\"accept-redaction\"|\"custom-replacement\"|\"false-positive\"|\"acknowledge-unscanned\",replacementText?:string}}], ownerRedactions, title, summary, confirmed:true, audiencePolicy, and optional expiresAt. Owner redactions are exact-text rules for content the owner considers sensitive even when the scanner does not. acknowledge-unscanned is only for a separately accepted unscannable-content warning, not a substitute for resolving detected secrets. Use [] if there are no findings or owner redactions. Do not send transcript, artifacts, or harness metadata.",
